...
if (isProd) {
  ...
  const buildProcess = spawn('npx', ['nuxi', 'build'], { stdio: 'inherit', shell: true, env: studioEnv })

  buildProcess.on('close', (code) => {
    if (code === 0) {
      spawn('npx', ['nuxi', 'preview', '--port', port], { stdio: 'inherit', shell: true, env: studioEnv })
    }
  })
} else {
  console.log(`\n✨ Starting in Local Dev Mode (Direct File Save) on port ${port}...\n`)
  spawn('npx', ['nuxi', 'dev', '--port', port], { stdio: 'inherit', shell: true, env: studioEnv })
}
export {}
